...
// 国密加密（支持SM4_OFB/CFB/CBC/ECB）
/**
 * @brief 使用对称密钥加密给定的明文数据。
 *
 * 此函数使用指定的对称加密算法和密钥对输入的明文数据进行加密。
 * 加密操作基于指定的初始化向量（IV）进行。
 *
 * @param [in] db_name         数据库名称，用于标识在哪个数据库中使用此加密操作。
 * @param [in] table_name      表名称，用于标识表数据的来源。
 * @param [in] key_id          密钥标识符，指定要使用的加密密钥。
 * @param [in] alg_id           对称加密算法标识符，指定使用的加密算法。
 * @param [in] iv              初始化向量（IV），用于加密操作。
 * @param [in] data             输入参数，指向需要加密的明文数据。
 * @param [in] data_length      明文数据的长度（字节数）。
 * @param [out] cipher_data      输出参数，指向存储加密后的密文数据的缓冲区。
 * @param [out] cipher_data_len 实际生成的密文数据长度（字节数）。
 *
 * @return                     返回加密操作的状态代码：
 *                             - 0: 成功
 *                             - 非零值: 错误代码，指示具体错误原因。
...
/**
 * @brief 使用对称密钥解密给定的加密数据。
 *
 * 此函数使用指定的对称加密算法和密钥对加密数据进行解密。
 * 解密操作基于指定的初始化向量（IV）进行。
 *
 * @param [in] db_name         数据库名称，用于标识在哪个数据库中使用此解密操作。
 * @param [in] table_name       表名称，用于标识表数据的来源。
 * @param [in] key_id          密钥标识符，指定要使用的解密密钥。
 * @param [in] alg_id           对称加密算法标识符，指定使用的加密算法。
 * @param [in] iv              初始化向量（IV），用于解密操作。
 * @param [in] cipher_data      输入参数，指向需要解密的加密数据。
 * @param [in] cipher_data_len 加密数据的长度（字节数）。
 * @param [out] plain_data      输出参数，指向存储解密后的明文数据的缓冲区。
 * @param [out] plain_data_len 实际解密得到的明文数据长度（字节数）。
 *
 * @return                     返回解密操作的状态代码：
 *                             - 0: 成功
 *                             - 非零值: 错误代码，指示具体错误原因。
...
/**
 * @brief 计算给定数据的 SM3 HMAC 值。
 *
 * 此函数使用 SM3 哈希算法和指定的密钥对输入数据进行 HMAC（哈希消息认证码）计算。
 * HMAC 可以用于验证数据的完整性和认证性。
 *
 * @param [in] db_name     数据库名称，用于标识在哪个数据库中使用此 HMAC。
 * @param [in] table_name  表名称，用于标识表数据的来源。
 * @param [in] key_id      密钥标识符，指定要使用的 HMAC 密钥。
 * @param [in] data        输入数据的指针，指向需要计算 HMAC 的数据。
 * @param [in] data_len    输入数据的长度（字节数）。
 * @param [out] hmac_value  输出参数，指向存储计算得到的 HMAC 值的缓冲区。
 * @param [out] hmac_len    实际生成的 HMAC 值的长度（字节数）。
 *
 * @return                  返回计算结果的状态代码：
 *                          - 0: 成功
 *                          - 非零值: 错误代码，指示具体错误原因。
 ...
 * @param[in] table_name 密钥所在表名
 * @param[in] id 密钥派生密钥id(根密钥id/主密钥id)
 ...
 * @param[in] tablename 密钥表名
 * @param[in] key      主密钥
 * @param[in] key_len  主密钥的长度
 * @param[in] as_name  AS名
 * @param[in] gs_name   GS名
